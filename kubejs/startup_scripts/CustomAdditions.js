...
StartupEvents.registry('block', allthemods => {
    allthemods.create('magical_soil').displayName('§b魔法土壤').grassSoundType().mapColor('grass').hardness(0.6);

    allthemods.create('air_essence_block').displayName('风之源质块').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('earth_essence_block').displayName('地之源质块').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('fire_essence_block').displayName('火之源质块').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('water_essence_block').displayName('水之源质块').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();

    allthemods.create('blaze_powder_block').displayName('烈焰粉块').stoneSoundType().mapColor('stone').hardness(1);
    // allthemods.create('blaze_block').displayName('Blaze Block').stoneSoundType().mapColor('stone').hardness(1);
    allthemods.create('saltpeter_block').displayName('硝石块').stoneSoundType().mapColor('stone').hardness(1);
})

StartupEvents.registry('item', allthemods => {
    
    if (Platform.isLoaded("eternal_starlight")) {
        allthemods.create('starlight_prediction').displayName('通用星光预言');
    }

    allthemods.create('silent_allthemodium_plate').displayName('寂静Allthemodium板').glow(true);
    ...
    allthemods.create('atm_star_shard_1').displayName('ATM之星碎片')
    allthemods.create('atm_star_shard_2').displayName('ATM之星碎片')
    allthemods.create('atm_star_shard_3').displayName('ATM之星碎片')
    allthemods.create('atm_star_shard_4').displayName('ATM之星碎片')
    allthemods.create('atm_star_shard_5').displayName('ATM之星碎片')
    ...
    allthemods.create("allthemodium_solar_sail_package").displayName("ATM太阳帆包裹").component("dysoncubeproject:sphere_component_solar_sail", Java.cast("java.lang.Integer", 64))
    allthemods.create("allthemodium_beam_package").displayName("ATM光束包裹").component("dysoncubeproject:sphere_component_beam", Java.cast("java.lang.Integer", 16))
...
global.iceAndFirePearls = (/** @type {$ServerLevel_}} */ level, /** @type {$Player_}} */ player, /** @type {$InteractionHand_}} */ hand) => {
    ...
    if (closest != null || structureResult != null) {
        ...
        player.statusMessage = "附近未找到龙或龙巢穴……"
        // console.log(`IceAndFire Pearl event took: ${currentStopwatch.elapsed("milliseconds")} ms`)
        return false
    }
}
