...
Ponder.registry((allthemods) => {
	...
	.scene('fission_mek','通用机械裂变反应堆', 'kubejs:fission_mek',
	...
	(scene, util) => {
			...
			scene.text(80, '边框必须是外壳', [0, 4.5, 4.5]).placeNearTarget();
			...
			scene.text(80, '墙面可以是反应堆外壳或玻璃', [0, 2.5, 2.5]).placeNearTarget();
			...
			scene.text(60, '在内部放置燃料组件方块以制成燃料棒', [2.5, 2.5, 2.5]).placeNearTarget();
			...
			scene.idle(10)
			scene.text(60, '在每根燃料棒顶部放置控制棒组件', [2.5, 3.5, 2.5]).placeNearTarget();
			scene.idle(20);
			scene.addKeyframe();
			scene.idle(60);
