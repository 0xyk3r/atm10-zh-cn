...
- **撞名 → 删掉整合包的翻译**。安装器直接覆盖同名文件，本包的 `aether.snbt` 只有 2 个键，
  盖掉了整合包同名文件的 167 个键；`building_tips.snbt` 3 键盖掉 257 键；
  `mekanism_reactors.snbt` 4 键盖掉 287 键。29 个 delta 里 **23 个撞名**。
...
- 三平台安装器跑了端到端回归：整合包的 169 行原文件安装后原样保留，2 个旧残留被清掉。
